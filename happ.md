# Happ (альтернатива для всех ОС)

Альтернативное приложение. Подходит для iOS, iPadOS, Android, macOS и Windows.

Основная инструкция для вашей ОС: [iOS / iPadOS](ios.md) · [Android](android.md) · [macOS](macos.md) · [Windows](windows.md)

## Шаг 1. Скачайте и установите приложение

- [iOS / iPadOS / macOS - App Store](https://apps.apple.com/kz/app/happ-proxy-utility/id6504287215)
- [Android - Google Play](https://play.google.com/store/apps/details?id=com.happproxy)
- [Windows - установщик](https://github.com/Happ-proxy/happ-desktop/releases/latest/download/setup-Happ.x64.exe)

В российском App Store Happ нет, поэтому на iPhone, iPad и Mac сначала [смените регион App Store](app-store-region.md).

## Шаг 2. Добавьте профиль

1. Скопируйте ключ, который я отправил
2. На главном экране нажмите кнопку **Из Буфера** внизу слева

<img src="images/happ1.jpg" width="400">

## Шаг 3. Включите фрагментирование и шумы

1. Откройте **Настройки** приложения
2. В разделе **ТУННЕЛЬ** включите переключатели **Использовать фрагментирование** и **Включить Шумы**

Эти настройки разбивают трафик на мелкие части и добавляют маскирующий "шум", что помогает обходить блокировки и DPI. Остальные значения менять не нужно - они подставляются автоматически.

<img src="images/happ2.jpg" width="400">

## Шаг 4. Подключитесь

Нажмите большую круглую кнопку питания в центре экрана.

## Важно

Не выбирайте ключи с пометкой **Аварийный**. Используйте их только в случае, если не работает ни один из обычных ключей.

## Полезное

- [Как обновить подписку](update-subscription.md)
- [Не работает VPN?](troubleshooting.md)

---

[← К выбору операционной системы](README.md)
